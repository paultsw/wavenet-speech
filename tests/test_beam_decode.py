...
logits = torch.randn(sequence_length, batch_size, ndim)
...
for k in range(sequence_length):
    label_lkhd = logits[k].view(batch_size, ndim).transpose(0,1).contiguous().unsqueeze(1).expand(8,4,8)
    # update each beam:
...
num_best = 2 # (only return the top-2 best hypothesis sequence for each batch)
hypotheses = {}
probas = {}
for b in range(batch_size):
