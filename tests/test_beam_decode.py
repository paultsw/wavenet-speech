...
sequence_length = 44
ndim = 7 #8 = { - A G C T <PAD> <S> </S> }
logits = torch.nn.functional.normalize(torch.randn(sequence_length, batch_size, ndim).abs_(), p=1, dim=2)
logits[:,:,5] = 0. # <BOS> == 0 for all timesteps
logits[:,:,6] = 0. # <EOS> == 0 for all timesteps

### add <START> and <STOP> tags to the logits:
start_vec = torch.Tensor([0,0,0,0,0,1,0]).view(1,1,ndim).expand(1, batch_size, ndim)
stop_vec = torch.Tensor([0,0,0,0,0,0,1]).view(1,1,ndim).expand(1, batch_size, ndim)
logits = torch.cat([start_vec, logits, stop_vec], dim=0)
sequence_length += 2

### loop through each timestep and update the respective beams:
for k in range(sequence_length):
    label_lkhd = logits[k].view(batch_size, ndim)
